...
for param in parameters_list:
    try:
        model = ARMA(df_month.Count,order=(param[0], param[1])).fit()
    except ValueError:
        print('参数错误:', param)
        continue
    aic = model.aic
...
df_month2.Count.plot(label='实际人数')
df_month2.forecast.plot(color='r', ls='--', label='预测人数')
plt.legend()
plt.title('乘客人数（月）')
plt.xlabel('时间')
plt.ylabel('指数')
